...
message when there were modifications made, undef if the message has
...
=default extra_rules []
The standard set of rules, which is the default for the P<rules> option,
...
the extra rule of C<removeDeletedParts> returns an undef, which
...
the rules will start all over again, however undef will immediately
stop it.
...
undef if the part should be removed, and any newly constructed
